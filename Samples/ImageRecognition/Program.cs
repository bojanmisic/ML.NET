namespace ImageRecognition
{
    ...
    internal class Program
    {
        ...
        {
            // Model downloaded from https://storage.googleapis.com/download.tensorflow.org/models/inception5h.zip
            ...
            using (var environment = new ConsoleEnvironment())
            {
                ...
                var loader = TextLoader.CreateReader(
                    environment, 
                    context => (
                ...
                var estimator = loader.MakeNewEstimator()
                    ...
                    .Append(row => (
                        row.Name, 
                        output2: row.input.ApplyTensorFlowGraph(tensorflowModel)));

                var model = estimator.Fit(data);

                var predictionFunction = model.AsDynamic.MakePredictionFunction<InceptionData, InceptionPrediction>(environment);

                var prediction = predictionFunction.Predict(new InceptionData() {ImagePath = "snail.jpg"});
